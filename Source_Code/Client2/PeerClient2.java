...
public class PeerClient2 extends Thread{
	...
	private static void openFileDownloadConnection() {
		...
		try {
			peerServer = new ServerSocket(5002);
			while(true){
				DownloadFileThread2 dlFile;
				//server.accept returns a client connection
				dlFile = new DownloadFileThread2(peerServer.accept());
		...
		try {
			...
			if(inputObject != null) {
				if(inputObject instanceof ArrayList) {
					...
					//If list contains one or more peers then dispaly it else file does not exits.
	...
	private static HashMap gatherMyInfo() {
		...
		try {
			...
			client1Map.put("PortNo",5002);
			client1Map.put("PeerId","Client2");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
