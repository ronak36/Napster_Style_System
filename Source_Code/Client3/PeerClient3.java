...
public class PeerClient3 extends Thread{
	...
	private static void openFileDownloadConnection() {
		...
		try {
			peerServer = new ServerSocket(5003);
			while(true){
				DownloadFileThread3 dlFile;
				//server.accept returns a client connection
				dlFile = new DownloadFileThread3(peerServer.accept());
	...
	private static HashMap gatherMyInfo() {
		...
		try {
			...
			client1Map.put("PortNo",5003);
			client1Map.put("PeerId","Client3");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
